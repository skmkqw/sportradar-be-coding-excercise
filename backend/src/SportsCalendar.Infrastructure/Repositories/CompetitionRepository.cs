...
public class CompetitionRepository : ICompetitionRepository
{
    ...
    public async Task<Competition?> GetByIdAsync(Guid id, IDbTransaction? transaction, CancellationToken ct = default)
    {
        const string sql = "SELECT * FROM Competitions WHERE Id = @Id";
        
        return await _dbConnection.QueryFirstOrDefaultWithTokenAsync<Competition>(sql, transaction, new { Id = id }, ct);
    }
